...
#library(ggpubr) <biblioteka do łączenia wykresów w ggplot, ale coś nie działa>
...
<<echo=F, fig.width=14, fig.height=5, fig.align="center", fig.cap="podpis rysunku">>=
#p1 <- ggplot(df2, aes(x=HEIGHT, y=REB, na.rm=TRUE, color=POSITION, shape=POSITION)) +
#     geom_jitter(size=4, stroke=1.4, alpha=0.4, na.rm=TRUE, width=0.01) +
#     scale_color_manual(values=MY_PALETTE1[1:4]) +
#     scale_shape_manual(values=c(1, 2, 5, 6)) +
#     guides(color = guide_legend(override.aes = list(size = 10))) +
#     labs(x = "wzrost", 
#          y = "asysty / mecz", 
#          subtitle = "Zbiórki vs. wzrost") +
#     theme(text = element_text(size=35))
#p2 <-  ggplot(df2, aes(x=HEIGHT, y=PTS, na.rm=TRUE, color=POSITION, shape=POSITION)) +
#     geom_jitter(size=4, stroke=1.4, alpha=0.4, na.rm=TRUE, width=0.01) +
#     scale_color_manual(values=MY_PALETTE1[1:4]) +
#     scale_shape_manual(values=c(1, 2, 5, 6)) +
#     guides(color = guide_legend(override.aes = list(size = 10))) +
#     labs(x = "wzrost", 
#          y = "asysty / mecz", 
#          subtitle = "Punkty vs. wzrost") +
#     theme(text = element_text(size=35))
#p3 <- ggplot(df2, aes(x=HEIGHT, y=AST, na.rm=TRUE, color=POSITION, shape=POSITION)) +
#     geom_jitter(size=4, stroke=1.4, alpha=0.4, na.rm=TRUE, width=0.01) +
#     scale_color_manual(values=MY_PALETTE1[1:4]) +
#     scale_shape_manual(values=c(1, 2, 5, 6)) +
#     guides(color = guide_legend(override.aes = list(size = 10))) +
#     labs(x = "wzrost", 
#          y = "asysty / mecz", 
#          subtitle = "Asysty vs. wzrost") +
#     theme(text = element_text(size=35))
#ggarrange(p1, p2, p3, ncol=1, nrow=3)
@  

Na wykresach widzimy, że biorąc za punkt odniesienia wzrost, ciężko nam będzie wyznaczyć najskuteczniejszych graczy. Chociaż można podejrzewać, że wzrost ma istotny wpływ na skuteczność w zbiórkach, to z jakichś przyczyn wyżsi gracze zdają się gorzej asystować. W zdobywaniu punktów wzrost zaś zdaje się w ogóle nie mieć znaczenia.

<<echo=F, fig.width=19, fig.height=5, fig.align="center", fig.cap="podpis rysunku">>=
# 
